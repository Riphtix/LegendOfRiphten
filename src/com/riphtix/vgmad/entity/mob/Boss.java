...
public class Boss extends Mob{
	...
	private AnimatedSprite down = new AnimatedSprite(SpriteSheet.dragon_down, 96, 96, 4);
	private AnimatedSprite up = new AnimatedSprite(SpriteSheet.dragon_up, 96, 96, 4);
	private AnimatedSprite left = new AnimatedSprite(SpriteSheet.dragon_left, 96, 96, 4);
	private AnimatedSprite right = new AnimatedSprite(SpriteSheet.dragon_right, 96, 96, 4);
	...
	public Boss(int x, int y, int rank, Classification classification){
		this.x = x << 4;
		this.y = y << 4;
		this.rank = rank;
		this.classification = classification;
		sprite = animSprite.getSprite();
		hitbox = new MobHitbox(Sprite.hitbox68x61);
		...
		maxHealth = Experience.calculateMobHealth(this, 800);
		health = maxHealth;
	...
	public void bossDamaged(double damage){
		...
		if (isDead()) {
			Sound.SoundEffect.DRAGON_ROAR.play();
	...
	protected void shootRandom() {
		...
		if (rand != null) {
			if (!(rand instanceof Particle) && !(rand instanceof ParticleSpawner) && rand instanceof Player && firerate <= 0) {
				...
				if(inventory.contains(dragonHeart)) {
					shoot(x, y, dir, this );
					firerate = DragonProjectile.FIRE_RATE;
				}
			}
		}
	}
	...
	public void render(Screen screen) {
		sprite = animSprite.getSprite();
		screen.renderMob((int) x - 48, (int) y - 48, sprite);
		hitbox.render((int) x - 34, (int) y - 19, screen);
	}
